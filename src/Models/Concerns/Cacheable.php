...
use Laravel\Subscriptions\Models\Contracts\ShouldCache;
...
trait Cacheable
{
    ...
    public static function bootCacheable(): void
    {
        static::updated(function (Model $model) {
            if ($model instanceof ShouldCache && $model->isCacheClearEnabled()) {
                $model::forgetCache();
            }
        });
    ...
    public function cacheQuery($builder, array $columns, Closure $closure)
    {
        ...
        if (method_exists(app('cache')->getStore(), 'tags')) {
            if ($lifetime === -1) {
                $result = app('cache')->tags($modelName)->rememberForever($cacheKey, $closure);
            } else {
                $result = app('cache')->tags($modelName)->remember($cacheKey, $lifetime, $closure);
            }
            return $result;
        }

        if ($lifetime === -1) {
            $result = app('cache')->rememberForever($cacheKey, $closure);
        } else {
            $result = app('cache')->remember($cacheKey, $lifetime, $closure);
        }
